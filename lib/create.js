...
exports.createCode = async () => {
    ...
    //读取配置文件
    const config = {
        ...dynamicallyConfigPath(configPath),
        projectDir: cwd
    }
    success('读取配置文件成功。')
    config.projects
        .map(item => {
            return {
                ...config,
                projectToken: item.token,
                title: item.title
            }
        })
        .map(async item => {
            //获取yapi数据
            const yapi = new Yapi(item)
            const result = await yapi.getProjectData()
            //生成code
            const generator = new Generator(result, item)
            const tsgenerator = new TsGenerator(result, item)
            generator.generate()
